 ...
 *	  PolarDB mini transaction manager
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/logindex/polar_mini_transaction.c
 *
 *-------------------------------------------------------------------------
 */
/*
...
static proc_occupied_mini_trans_t proc_occupied_mini_trans =
{
	0
};
...
polar_logindex_abort_mini_transaction(mini_trans_t trans)
{
	...
	POLAR_ASSERT_PANIC(proc_occupied_mini_trans.trans == trans);
...
mini_trans_find(mini_trans_t trans, BufferTag *tag, uint32 key)
{
	...
	if (MINI_TRANS_IS_OCCUPIED(trans, key + 1))
	{
		...
		while (i != 0)
		{
			...
			POLAR_ASSERT_PANIC(MINI_TRANS_IS_OCCUPIED(trans, i));
								   ...
								   LWLockMode mode, XLogRecPtr *lsn)
{
	...
	if (lock)
	{
		...
		POLAR_ASSERT_PANIC(trans == proc_occupied_mini_trans.trans);
...
polar_logindex_mini_trans_set_page_added(mini_trans_t trans, polar_page_lock_t lock)
{
	...
	POLAR_ASSERT_PANIC(trans->info[i].added == false);
