...
log_index_table_saved_before_promote(log_index_snapshot_t * logindex_snapshot, log_mem_table_t * table)
{
	...
	if (unlikely(promoted_info->old_primary_saved_max_tid != LOG_INDEX_TABLE_INVALID_ID))
	{
		saved = LOG_INDEX_MEM_TBL_TID(table) <= promoted_info->old_primary_saved_max_tid;

		if (LOG_INDEX_MEM_TBL_TID(table) == promoted_info->old_primary_saved_max_tid)
		{
			if (table->data.max_lsn != promoted_info->old_primary_saved_max_lsn)
				elog(PANIC, "promote last table max_lsn=%lX, max lsn saved by old primary is %lX",
					 table->data.max_lsn, promoted_info->old_primary_saved_max_lsn);
		}
...
log_index_flush_table(log_index_snapshot_t * logindex_snapshot, XLogRecPtr checkpoint_lsn, bool flush_active)
{
	...
	do
	{
			 ...
			 !LOG_INDEX_MEM_TBL_IS_NEW(table)))
		{
			 ...
			 * During crash recovery GetFlushRecPtr return invalid value, so
			 * we compare with GetXLogReplayRecPtr().
			 */
			if ((LOG_INDEX_MEM_TBL_STATE(table) == LOG_INDEX_MEM_TBL_STATE_INACTIVE ||
				 LOG_INDEX_MEM_TBL_STATE(table) == LOG_INDEX_MEM_TBL_STATE_ACTIVE)
			...
			{
				/*
				 * After promoting don't write table which was flushed by
				 * previous primary
				 */
				...
				if (succeed)
				{
					/* save last flush logindex max_lsn for active table */
					if (flush_active)
						last_flush_max_lsn = table->data.max_lsn;
		...
		/*
		 * If checkpoint lsn is valid we will flush all table which state is
		 * INACTIVE and table's lsn is smaller than checkpoint lsn. If
		 * consistent lsn is larger than max saved logindex lsn, we will try
		 * to flush table until saved logindex lsn is larger than consistent
		 * lsn. Otherwise we will try to flush all INACTIVE table, but don't
		 * flush table number more than polar_logindex_table_batch_size.
		 */
		SpinLockAcquire(LOG_INDEX_SNAPSHOT_LOCK);
...
static bool
log_index_primary_bg_write(log_index_snapshot_t * logindex_snapshot)
{
	return log_index_flush_table(logindex_snapshot, InvalidXLogRecPtr, false);
}

void
polar_logindex_flush_table(logindex_snapshot_t logindex_snapshot, XLogRecPtr checkpoint_lsn, bool flush_active)
{
	log_index_flush_table(logindex_snapshot, checkpoint_lsn, flush_active);
}
...
static void
log_index_update_replica_table_state(log_index_snapshot_t * logindex_snapshot, log_idx_table_id_t max_saved_tid)
{
	uint32		mid,
				start_mid;
	log_mem_table_t *table;
	...
	start_mid = (max_saved_tid - 1) % logindex_snapshot->mem_tbl_size;
	mid = start_mid;
	...
	do
	{
		...
		/* All the inactive tables in memory are arranged one by one. */
		else
			break;
...
log_index_replica_bg_write(log_index_snapshot_t * logindex_snapshot)
{
	...
	log_idx_table_id_t max_tid,
				max_saved_tid = LOG_INDEX_TABLE_INVALID_ID;
	...
	{
		/*
		 * Return true if no new table is added and no table state need to be
		 * updated
		 */
		return true;
	}
	...
	log_index_update_replica_table_state(logindex_snapshot, max_saved_tid);
...
polar_logindex_bg_write(logindex_snapshot_t logindex_snapshot)
{
	...
	if (likely(polar_logindex_check_state(logindex_snapshot, POLAR_LOGINDEX_STATE_ADDING)))
	{
		if (!polar_logindex_check_state(logindex_snapshot, POLAR_LOGINDEX_STATE_WRITABLE))
			write_done &= log_index_replica_bg_write(logindex_snapshot);
		else
			write_done &= log_index_primary_bg_write(logindex_snapshot);
	}
...
static void
log_index_init_lwlock(log_index_snapshot_t * logindex_snapshot, int offset, int size, int tranche_id)
{
	int			i,
				j;
...
logindex_snapshot_init_promoted_info(log_index_snapshot_t * logindex_snapshot)
{
	...
	info->old_primary_saved_max_lsn = InvalidXLogRecPtr;
	info->old_primary_max_inserted_lsn = InvalidXLogRecPtr;
	info->old_primary_max_tid = LOG_INDEX_TABLE_INVALID_ID;
	info->old_primary_saved_max_tid = LOG_INDEX_TABLE_INVALID_ID;
}
								  ...
								  TimeLineID replay_tli)
{
	...
	List	   *expected_tles = readTimeLineHistory(replay_tli);

	POLAR_ASSERT_PANIC(!polar_logindex_check_state(logindex_snapshot, POLAR_LOGINDEX_STATE_INITIALIZED));
	POLAR_ASSERT_PANIC(!XLogRecPtrIsInvalid(checkpoint_lsn));

	LWLockAcquire(LOG_INDEX_IO_LOCK, LW_EXCLUSIVE);

	/*
	 * Reset logindex when we can not get correct logindex meta from storage
	 * or meta->start_lsn is in different timeline with checkpoint_lsn.
	 */
	if (!log_index_get_meta(logindex_snapshot, meta) ||
		replay_tli != tliOfPointInHistory(meta->start_lsn, expected_tles))
	{
	...
	/*
	 * If meta->start_lsn is invalid, we will not parse xlog and save it to
	 * logindex. Replica will check logindex meta again when parse checkpoint
	 * xlog. Primary will set logindex meta start lsn and save to storage when
	 * create new checkpoint. This will make replica and primary to create
	 * logindex from the same checkpoint.
	 */
	...
	/*
	 * When we start to truncate lsn , latest_page_number may not be set up;
	 * insert a suitable value to bypass the sanity test in SimpleLruTruncate.
	 */
	...
	/*
	 * When initialize log index snapshot, we load max table id's data to
	 * memory. When first insert lsn to memory table, need to check whether it
	 * already exists in previous table
	 */
...
XLogRecPtr
polar_logindex_snapshot_init(logindex_snapshot_t logindex_snapshot, XLogRecPtr checkpoint_lsn,
							 TimeLineID checkpoint_tli, bool read_only, bool flush_active_table)
{
	...
	start_lsn = polar_logindex_snapshot_base_init(logindex_snapshot, checkpoint_lsn, checkpoint_tli);

	logindex_snapshot->flush_active_table = flush_active_table;
...
static void
log_index_init_lwlock_array(log_index_snapshot_t * logindex_snapshot, int tranche_id_begin, int tranche_id_end)
{
	int			tranche_id = tranche_id_begin;

	POLAR_ASSERT_PANIC(logindex_snapshot->mem_tbl_size > 0);

	/*
	 * The tranche id defined for logindex must map the following call
	 * sequence. See the definition of LWTRANCE_WAL_LOGINDEX_BEGIN and
	 * LWTRANCHE_WAL_LOGINDEX_END as example
	 */
	...
	log_index_init_lwlock(logindex_snapshot, LOG_INDEX_FLUSH_ACTIVE_TBL_LOCK_OFFSET, 1,
						  tranche_id++);

	log_index_init_lwlock(logindex_snapshot, LOG_INDEX_BLOOM_LRU_LOCK_OFFSET, 1,
						  tranche_id);

	POLAR_ASSERT_PANIC(tranche_id == tranche_id_end);
}
...
#define LOGINDEX_BLOOM_SUFFIX "_bloom"
	...
	POLAR_ASSERT_PANIC(logindex_snapshot != NULL);
	...
	{
		POLAR_ASSERT_PANIC(!found_snapshot && !found_locks);
		logindex_snapshot->mem_tbl_size = logindex_mem_tbl_size;

		pg_atomic_init_u32(&logindex_snapshot->state, 0);

		log_index_init_lwlock_array(logindex_snapshot, tranche_id_begin, tranche_id_end);
		...
		strlcpy(logindex_snapshot->dir, name, NAMEDATALEN);
	...
	/*
	 * Notice: When define tranche id for logindex, the last one is used for
	 * logindex bloom. See the definition between LWTRANCE_WAL_LOGINDEX_BEGIN
	 * and LWTRANCE_WAL_LOGINDEX_END
	 */
	SimpleLruInit(&logindex_snapshot->bloom_ctl, item_name,
				  bloom_blocks, 0,
				  LOG_INDEX_BLOOM_LRU_LOCK, name,
				  tranche_id_end, SYNC_HANDLER_NONE, true);
...
log_index_get_meta(log_index_snapshot_t * logindex_snapshot, log_index_meta_t * meta)
{
	...
	r = FileRead(fd, (char *) meta, sizeof(log_index_meta_t), 0, WAIT_EVENT_LOGINDEX_META_READ);
...
polar_log_index_write_meta(log_index_snapshot_t * logindex_snapshot, log_index_meta_t * meta, bool update)
{
	...
	if (FileWrite(fd, (char *) meta, sizeof(log_index_meta_t), 0x0, WAIT_EVENT_LOGINDEX_META_WRITE)
...
/*
 * Save table if it's running in primary node's recovery process.
 * During primary recovery, bgwriter is not started and we have to
...
log_index_wait_active(log_index_snapshot_t * logindex_snapshot, log_mem_table_t * table, XLogRecPtr lsn)
{
	...
	for (;;)
	{
		...
		/*
		 * We only save table to storage when polar_streaming_xlog_meta is
		 * true. If the table we are waiting is inactive then force to save it
		 * in this process.
		 */
		if (LOG_INDEX_MEM_TBL_STATE(table) == LOG_INDEX_MEM_TBL_STATE_INACTIVE)
...
log_index_mem_seg_full(log_mem_table_t * table, log_seg_id_t head)
{
	...
	POLAR_ASSERT_PANIC(head != LOG_INDEX_TBL_INVALID_SEG);
						  ...
						  log_seg_id_t new_item_id)
{
	...
	POLAR_ASSERT_PANIC(key < LOG_INDEX_MEM_TBL_HASH_NUM);
...
log_index_append_lsn(log_mem_table_t * table, log_seg_id_t head, log_index_lsn_t * lsn_info)
{
	...
	if (item->tail_seg == head)
	{
		POLAR_ASSERT_PANIC(item->number < LOG_INDEX_ITEM_HEAD_LSN_NUM);
		idx = item->number;
	...
	else
	{
		seg = log_index_item_seg(&table->data, item->tail_seg);
		POLAR_ASSERT_PANIC(seg != NULL);
		POLAR_ASSERT_PANIC(seg->number < LOG_INDEX_ITEM_SEG_LSN_NUM);
		idx = seg->number;
...
log_index_next_free_seg(log_index_snapshot_t * logindex_snapshot, XLogRecPtr lsn, log_mem_table_t * *active_table)
{
	...
	for (;;)
	{
		...
		{
			/*
			 * 1. However when we get a new active table, we don't know its
			 * data.prefix_lsn, we assign InvalidXLogRecPtr lsn to
			 * data.prefix_lsn, so we should distinguish which table is new
			 * without prefix_lsn, and reassign it 2. If active table is full
			 * or new lsn prefix is different than this table's lsn prefix we
			 * will allocate new active memory table.
			 */
...
static void
log_index_calc_bloom(log_idx_table_data_t * table_data, log_file_table_bloom_t * bloom)
{
	bloom_filter *filter;
	int			i;

	filter = polar_bloom_init_struct(bloom->bloom_bytes, bloom->buf_size,
									 LOG_INDEX_BLOOM_ELEMS_NUM, 0);

	for (i = 0; i < LOG_INDEX_MEM_TBL_HASH_NUM; i++)
	{
		log_seg_id_t id = LOG_INDEX_TBL_SLOT_VALUE(table_data, i);

		if (id != LOG_INDEX_TBL_INVALID_SEG)
		{
			log_item_head_t *item = log_index_item_head(table_data, id);

			while (item != NULL)
			{
				bloom->max_lsn = Max(bloom->max_lsn,
									 log_index_item_max_lsn(table_data, item));
				bloom->min_lsn =
					Min(bloom->min_lsn, LOG_INDEX_SEG_MIN_LSN(table_data, item));
				bloom_add_element(filter, (unsigned char *) &(item->tag),
								  sizeof(BufferTag));
				item = log_index_item_head(table_data, item->next_item);
			}
		}
	}
}
...
log_index_save_table(log_index_snapshot_t * logindex_snapshot, log_idx_table_data_t * table, File fd, log_file_table_bloom_t * bloom)
{
	...
	ret = FileWrite(fd, (char *) table, sizeof(*table), offset, WAIT_EVENT_LOGINDEX_TBL_WRITE);
...
static void
log_index_get_bloom_data(log_idx_table_data_t * table_data, log_idx_table_id_t tid, uint32 state, log_file_table_bloom_t * bloom)
{
	...
	log_index_calc_bloom(table_data, bloom);
...
log_index_save_bloom(log_index_snapshot_t * logindex_snapshot, log_idx_table_id_t tid, log_file_table_bloom_t * bloom)
{
	 ...
	 * O_CREAT flag will be set to open this file.Otherwise we will open this
	 * segment file without O_CREAT flag to append bloom data.
	 */
...
log_index_read_table_data(log_index_snapshot_t * logindex_snapshot, log_idx_table_data_t * table, log_idx_table_id_t tid, int elevel)
{
	...
	bytes = FileRead(fd, (char *) table, sizeof(log_idx_table_data_t), offset, WAIT_EVENT_LOGINDEX_TBL_READ);
...
log_index_write_table_data(log_index_snapshot_t * logindex_snapshot, log_mem_table_t * table)
{
	...
	log_idx_table_data_t *table_data = NULL;
	...
	/*
	 * When the checkpointer is writing the active logindex table, the wal
	 * writer may be inserting lsn into the logindex table, resulting in
	 * inconsistent crc, so use lwlock to prevent this case.
	 */
	if (LOG_INDEX_MEM_TBL_STATE(table) == LOG_INDEX_MEM_TBL_STATE_ACTIVE &&
		logindex_snapshot->flush_active_table)
	{
		table_data = palloc(sizeof(log_idx_table_data_t));
		LWLockAcquire(LOG_INDEX_FLUSH_ACTIVE_TBL_LOCK, LW_EXCLUSIVE);
		memcpy(table_data, &table->data, sizeof(log_idx_table_data_t));
		LWLockRelease(LOG_INDEX_FLUSH_ACTIVE_TBL_LOCK);
	}
	else
	...
	log_index_get_bloom_data(table_data, tid, LOG_INDEX_MEM_TBL_STATE(table), bloom);

	/*
	 * POLAR: we save bloom data before table data, in case that replica read
	 * bloom data with zero page
	 */
	log_index_save_bloom(logindex_snapshot, tid, bloom);
	ret = log_index_save_table(logindex_snapshot, table_data, fd, bloom);
	pfree(bloom);

	FileClose(fd);

	/* must pfree temporary active table memory */
...
log_index_read_seg_file(log_index_snapshot_t * logindex_snapshot, log_table_cache_t * cache, uint64 segno)
{
	...
	else
	{
		...
		bytes = FileRead(fd, cache->data, LOG_INDEX_TABLE_CACHE_SIZE, 0x0, WAIT_EVENT_LOGINDEX_TBL_READ);
	...
	/*
	 * If this segment file is renamed from previous segment file, maybe
	 * there're old tables in the end of the file. So we have to compare to
	 * get max table id, and compare with meta to check it's already full
...
log_index_read_table(logindex_snapshot_t logindex_snapshot, log_idx_table_id_t tid, log_mem_table_t * *mem_table)
{
	...
	*mem_table = NULL;
	...
	/*
	 * Replace the table with the same position in shared memory and its state
	 * is FLUSHED
	 */
...
log_index_write_table(log_index_snapshot_t * logindex_snapshot, log_mem_table_t * table)
{
	...
	if (SIMPLE_FAULT_INJECTOR("test_skip_write_logindex") == FaultInjectorTypeEnable)
		return true;
#endif
	...
	{
		bool		active_state_before_write = (LOG_INDEX_MEM_TBL_STATE(table) == LOG_INDEX_MEM_TBL_STATE_ACTIVE);
		...
		/*
		 * We don't update meta when flush active memtable, before the
		 * log_index_write_table_data function, the table maybe be active, but
		 * after write table, it has become inactive, so use
		 * active_state_before_write to record table status.
		 */
		else if (active_state_before_write)
		{
...
static void
log_index_truncate_tbl(log_index_snapshot_t * logindex_snapshot, uint64 min_segment_no, uint64 max_segment_no)
{
	char		path[MAXPGPATH];
...
static void
log_index_truncate_bloom(log_index_snapshot_t * logindex_snapshot, log_idx_table_id_t max_unused_tid)
{
	uint64		bloom_page;

	if (max_unused_tid != LOG_INDEX_TABLE_INVALID_ID)
	{
		bloom_page = LOG_INDEX_TBL_BLOOM_PAGE_NO(max_unused_tid);
		SimpleLruTruncate(&logindex_snapshot->bloom_ctl, bloom_page);
		elog(LOG, "logindex truncate all blooms before the one whose id=%ld, page=%ld", max_unused_tid, bloom_page);
	}
}
...
log_index_truncate(log_index_snapshot_t * logindex_snapshot, XLogRecPtr lsn)
{
	log_index_meta_t *meta = &logindex_snapshot->meta;
	log_index_file_segment_t *min_seg = &meta->min_segment_info;
	uint64		min_segment_no = 0;
	uint64		max_segment_no;
	log_idx_table_id_t max_unused_tid = LOG_INDEX_TABLE_INVALID_ID;
	...
	while (true)
	{
		...
		if (meta->crc == 0 || XLogRecPtrIsInvalid(meta->max_lsn) ||
			XLogRecPtrIsInvalid(min_seg->max_lsn) || min_seg->max_lsn >= lsn)
		{
			LWLockRelease(LOG_INDEX_IO_LOCK);
			break;
		}

		POLAR_ASSERT_PANIC(LOG_INDEX_FILE_TABLE_SEGMENT_NO(meta->max_idx_table_id) >= min_seg->segment_no);
		POLAR_ASSERT_PANIC(meta->max_idx_table_id >= min_seg->max_idx_table_id);
		...
		/*
		 * Update meta first. If meta update succeed but fail to remove files,
		 * we will not read these files. Otherwise if we remove files succeed
		 * but fail to update meta, we will fail to read file base on meta
		 * data.
		 */
		...
		LWLockRelease(LOG_INDEX_IO_LOCK);
		log_index_truncate_tbl(logindex_snapshot, min_segment_no, max_segment_no);
	}
	log_index_truncate_bloom(logindex_snapshot, max_unused_tid);
	polar_logindex_truncate_local_cache(logindex_snapshot, min_segment_no);
}
...
polar_logindex_truncate(log_index_snapshot_t * logindex_snapshot, XLogRecPtr lsn)
{
	if (!XLogRecPtrIsInvalid(lsn))
	{
		...
		log_index_truncate(logindex_snapshot, lsn);
...
log_index_insert_lsn(log_index_snapshot_t * logindex_snapshot, log_index_lsn_t * lsn_info, uint32 key)
{
	...
	bool		new_item;
	bool		need_flush_lock = logindex_snapshot->flush_active_table;
	log_seg_id_t head = LOG_INDEX_TBL_INVALID_SEG;

	POLAR_ASSERT_PANIC(lsn_info->prev_lsn < lsn_info->lsn);

	active = LOG_INDEX_MEM_TBL_ACTIVE();

	/*
	 * 1. Logindex table state is atomic uint32, it's safe to change state
	 * without table lock 2. Only one process insert lsn, so it's safe to
	 * check exists page without hash lock
	 */
	if (LOG_INDEX_MEM_TBL_STATE(active) == LOG_INDEX_MEM_TBL_STATE_ACTIVE &&
	...
	if (!new_item && !log_index_mem_seg_full(active, head))
	{
		...
		/*
		 * When adding lsn, the checkpointer may be flushing the active table
		 * to the disk, so we use a LWlock for concurrency control.
		 */
		if (need_flush_lock)
			LWLockAcquire(LOG_INDEX_FLUSH_ACTIVE_TBL_LOCK, LW_EXCLUSIVE);
	...
	else
	{
		...
		POLAR_ASSERT_PANIC(dst != LOG_INDEX_TBL_INVALID_SEG);
	...
	/*
	 * It must be placed at the end, because modifying active->data.max_lsn
	 * will also affect the calculation of table CRC.
	 */
	if (need_flush_lock)
		LWLockRelease(LOG_INDEX_FLUSH_ACTIVE_TBL_LOCK);
}
...
polar_logindex_add_lsn(log_index_snapshot_t * logindex_snapshot, BufferTag *tag, XLogRecPtr prev, XLogRecPtr lsn)
{
	...
	POLAR_ASSERT_PANIC(tag != NULL && lsn > prev);
	...
	{
		/*
		 * Insert logindex from meta->start_lsn, so We don't save logindex
		 * which lsn is less then meta->start_lsn
		 */
		if (lsn < meta->start_lsn)
			return;

		/*
		 * If log index initialization is not finished we don't save lsn if
		 * it's less than max saved lsn, which means it's already in saved
		 * table
		 */
		if (lsn < meta->max_lsn)
			return;
		...
		/*
		 * If lsn is equal to max saved lsn then we check whether the tag is
		 * in saved table
		 */
		if (meta->max_lsn == lsn
		...
		/*
		 * If we come here which means complete to check lsn overlap then we
		 * can save lsn to logindex memory table
		 */
...
polar_load_logindex_snapshot_from_storage(log_index_snapshot_t * logindex_snapshot, XLogRecPtr start_lsn)
{
	...
	int			mid = 0;
	log_idx_table_id_t tid = 0,
				min_tid = 0;
	uint32		state = pg_atomic_read_u32(&logindex_snapshot->state);

	/*
	 * If it's Primary node, we only load tables whose min_lsn is less than
	 * start_lsn. But if we enable wal prefetch, we will create lsn iterator
	 * during recovery, so loading flushed table into memory can improve
	 * performance in some degree. If it's replica node, we will load all
	 * saved table from max_idx_table_id to memory table. It's optimization
	 * for the following page iterator, saving time to load table from
	 * storage.
	 */
	...
	/*
	 * POLAR: if we are initializing now, we load tables from big to small we
	 * cannot load all tables, because memtable is limited, so we should
	...
	POLAR_ASSERT_PANIC(min_tid != LOG_INDEX_TABLE_INVALID_ID);
	...
	if (new_max_idx_table_id >= min_tid)
	{
		...
		while (tid >= min_tid)
		{
			...
			if (!log_index_read_table_data(logindex_snapshot, &table, tid, LOG))
			{
				POLAR_LOG_LOGINDEX_META_INFO(meta);
				ereport(PANIC,
						(errmsg("Failed to read log index which tid=%ld when load logindex", tid)));
			}
			...
			POLAR_ASSERT_PANIC(table.idx_table_id == tid);
			tid--;
	...
	/*
	 * If logindex is initialized we will try to read active table data from
	 * storage. When we flush active table data to storage we will not update
	 * logindex meta
	 */
	if (!(state & POLAR_LOGINDEX_STATE_INITIALIZED))
		return;

	POLAR_ASSERT_PANIC(active->data.idx_table_id != LOG_INDEX_TABLE_INVALID_ID);
		...
		table.idx_table_id == LOG_INDEX_MEM_TBL_TID(active))
	{
		...
		SpinLockAcquire(LOG_INDEX_SNAPSHOT_LOCK);
		POLAR_ASSERT_PANIC(active->data.max_lsn >= logindex_snapshot->max_lsn);
		logindex_snapshot->max_idx_table_id = LOG_INDEX_MEM_TBL_TID(active);
		logindex_snapshot->max_lsn = active->data.max_lsn;
		SpinLockRelease(LOG_INDEX_SNAPSHOT_LOCK);

		/*
		 * When we read active memtable from storage, maybe free_head is too
		 * old
		 */
...
{
	return (mem_size * 1024L * 1024L) / (sizeof(log_mem_table_t) + sizeof(polar_lwlock_mini_padded));
}
...
void
polar_logindex_online_promote(logindex_snapshot_t logindex_snapshot, bool missing_ok)
{
	log_index_meta_t *meta = &logindex_snapshot->meta;
	SlruShared	shared = logindex_snapshot->bloom_ctl.shared;
	log_index_promoted_info_t *promoted_info = &logindex_snapshot->promoted_info;
	...
	if (log_index_get_meta(logindex_snapshot, meta))
	{
		promoted_info->old_primary_saved_max_lsn = meta->max_lsn;
		promoted_info->old_primary_saved_max_tid = meta->max_idx_table_id;
		promoted_info->old_primary_max_tid = meta->max_idx_table_id;
	...
	shared->polar_replica_promoting = true;
	pg_atomic_fetch_or_u32(&logindex_snapshot->state, POLAR_LOGINDEX_STATE_WRITABLE);
	LWLockRelease(LOG_INDEX_IO_LOCK);
...
polar_logindex_check_valid_start_lsn(logindex_snapshot_t logindex_snapshot)
{
	...
	{
		/*
		 * When reach consistency in replica node, read meta from storage is
		 * meta start lsn is invalid
		 */
		if (!polar_logindex_check_state(logindex_snapshot, POLAR_LOGINDEX_STATE_WRITABLE))
		{
			if (log_index_get_meta(logindex_snapshot, meta))
...
polar_logindex_update_promoted_info(logindex_snapshot_t logindex_snapshot, XLogRecPtr last_replayed_lsn)
{
	...
	info->old_primary_max_inserted_lsn = last_replayed_lsn;
	info->old_primary_max_tid = logindex_snapshot->max_idx_table_id;
}
...
/*
 * POLAR: total_segment_size just an estimated value, it's very expensive
 * to get real total_segment_size, but it's no need real value, it's just
 * a hint(we will try again later)
 */
uint64
polar_get_estimate_fullpage_segment_size(logindex_snapshot_t logindex_snapshot)
{
	uint64		estimate_total_segment_size = 0;
	log_idx_table_id_t max_tid = 0,
				min_tid = 0;

	SpinLockAcquire(LOG_INDEX_SNAPSHOT_LOCK);
	max_tid = logindex_snapshot->max_idx_table_id;
	min_tid = logindex_snapshot->meta.min_segment_info.min_idx_table_id;
	SpinLockRelease(LOG_INDEX_SNAPSHOT_LOCK);

	/*
	 * NOTE: we assume a logindex table contains 4096 fullpages, so segment
	 * size is 4096 * 8192 * table_num
	 */
	estimate_total_segment_size = 1L * (LOG_INDEX_MEM_TBL_SEG_NUM * BLCKSZ / (1024 * 1024L))
		* (max_tid - min_tid);
	return estimate_total_segment_size;
}
