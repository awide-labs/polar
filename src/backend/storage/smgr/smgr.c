 ...
 * backend.  If you call smgropen() twice for the same rel node, you get a
...
/*
 * smgrreleaserelnode() -- Release resources for given RelFileNode, if
 *						   it's open.
 *
 * This has the same effects as smgrrelease(smgropen(rnode)), but avoids
 * uselessly creating a hashtable entry only to drop it again when no
 * such entry exists already.
 */
void
smgrreleaserelnode(RelFileNodeBackend rnode)
{
	SMgrRelation reln;
